...
resource "snowflake_user" "core" {
  ...
  default_role         = lookup(each.value, "default_role", var.default_user_role)
  default_namespace    = lookup(each.value, "default_namespace", var.default_user_namespace)
  default_warehouse    = lookup(each.value, "default_warehouse", var.default_user_warehouse)
